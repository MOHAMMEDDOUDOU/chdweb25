...
export default function SoinsPage() {
  ...
  useEffect(() => {
    async function fetchProducts() {
      setLoading(true)
      const { data, error } = await supabase.from("products").select("*").eq("category", "Soins visage")
        ...
        <div className="container mx-auto px-4">
          <AnimatedSection direction="up" delay={100}>
            <div className="text-center mb-12">
              <h1 className="text-3xl font-extrabold text-[#b89b6a] mb-2">Soins visage</h1>
              <p className="text-base text-gray-500 mb-6">Soins visage pour une peau éclatante et saine.</p>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {products.map((product, index) => (
                <AnimatedSection key={product.id} direction="up" delay={index * 100}>
                  <GlassContainer className="bg-white/8 backdrop-blur-lg border-[#C29B87]/20 rounded-2xl shadow-2xl shadow-black/40 p-6 hover:scale-105 transition-transform duration-300">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex gap-2 mt-4 w-full">
                        <Button
                          className="flex-1 bg-gradient-to-r from-orange-500 to-red-600 hover:from-orange-600 hover:to-red-700 text-white font-bold rounded-full shadow-lg text-lg py-3 transition-transform duration-200 hover:scale-105"
